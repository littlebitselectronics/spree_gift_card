...
describe Spree::GiftCard do
  ...
  context '#apply' do
    ...
    it 'creates adjustment with mandatory set to true' do
      ...
      order.reload
      order.adjustments.where(source_id: gift_card.id, source_type: gift_card.class.to_s).first.mandatory.should be_true
    end
    ...
    context 'for order total larger than gift card amount' do
      it 'creates adjustment for full amount' do
        ...
        order.update!
        gift_card.apply(order)
        order.adjustments.where(source_id: gift_card.id, source_type: gift_card.class.to_s).first.amount.to_f.should eql(-25.0)
      end
    end
    ...
    context 'for order total smaller than gift card amount' do
      it 'creates adjustment for order total' do
        ...
        order.adjustments.where(source_id: gift_card.id, source_type: gift_card.class.to_s).first.amount.to_f.should eql(-10.0)
      end
    end
  end
